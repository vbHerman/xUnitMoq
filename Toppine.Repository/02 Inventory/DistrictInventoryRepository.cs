...
namespace Toppine.Repository
{
    public class DistrictInventoryRepository : BaseRepository<DistrictInventory>, IDistrictInventoryRepository
    {
        ...
        public DistrictInventoryRepository(DbContext dbContext) : base(dbContext)
        {
        }
        /// <summary>  
        /// 获取地区库存下拉列表（含缓存和权限过滤）  
        ...
        public async Task<List<IntIDDescriptionDto>> GetItemDropAsync(BaseDto para = null)
        {
            ...
            List<IntIDDescriptionDto> itemList = ManualDataCache.Instance.Get<List<IntIDDescriptionDto>>(cacheKey);
            if (itemList == null)
            {
                // 2. 缓存未命中，从数据库查询  
                itemList = await _db.DistrictInventory
